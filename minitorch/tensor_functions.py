...
class Permute(Function):
    @staticmethod
    def forward(ctx: Context, a: Tensor, order: Tensor) -> Tensor:
        # order_change = order.to_numpy().astype(int).tolist()
        # ctx.save_for_backward(order_change)
        # return a._new(a._tensor.permute(*order_change))
        ctx.save_for_backward(order)
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        # return grad_output._new(grad_output._tensor.permute(*np.argsort(order))), 0.0

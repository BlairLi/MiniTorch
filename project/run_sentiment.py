...
class CNNSentimentKim(minitorch.Module):
    ...
    def forward(self, embeddings):
        ...
        out1 = self.conv1d1.forward(embeddings).relu()
        out2 = self.conv1d2.forward(embeddings).relu()
        out3 = self.conv1d3.forward(embeddings).relu()
        out = minitorch.max(out1, 2) + minitorch.max(out2, 2) + minitorch.max(out3, 2)
